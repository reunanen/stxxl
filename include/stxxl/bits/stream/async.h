 ...
 *
 *  Copyright (C) 2008 Johannes Singler <singler@kit.edu>
 *
...
#ifndef STXXL_STREAM_ASYNC_HEADER
#define STXXL_STREAM_ASYNC_HEADER
...
//! Sub-namespace for providing async pipeline processing.
namespace async {
...
//! Helper class encapsulating a buffer.
...
public:
    ...
    value_type* end;
    //! In case the buffer is not full, stop may differ from end.
    value_type* stop;
    //! Currrent read or write position.
    value_type* current;
...
public:
    ...
    basic_pull_empty(StreamOperation& so, StartMode start_mode)
...
//! Helper function to call basic_pull_empty::async_pull() in a thread.
template <class StreamOperation>
...
{
public:
    pull_empty(StreamOperation& so, StartMode start_mode = STXXL_START_PIPELINE_DEFERRED_DEFAULT)
        : basic_pull_empty<StreamOperation>(so, start_mode)
    {
        if (start_mode != start_deferred)
...
protected:
    ...
    using base::so;
    using base::start_mode;

    //! Asynchronous method that keeps trying to fill the incoming buffer.
...
{
public:
    pull_empty_batch(StreamOperation& so, StartMode start_mode = STXXL_START_PIPELINE_DEFERRED_DEFAULT)
        : basic_pull_empty<StreamOperation>(so, start_mode)
    {
        if (start_mode != start_deferred)
            basic_pull_empty<StreamOperation>::start_pulling();
...
//! Helper node for asynchronous pipelining.
//!
...
protected:
    //! First double buffering buffer.
    typed_buffer block1;
    //! Second double buffering buffer.
    typed_buffer block2;
    //! Buffer that is currently input to.
    ...
    //! Mutex variable, to mutually exclude the other thread.
...
protected:
    //! Check whether incoming buffer has run full and wait for data being pulled.
    void offload() const
    {
...
public:
    ...
    //! \param buffer_size Total size of the buffers in bytes.
    //! \param so Input stream operation.
    basic_pull(unsigned_type buffer_size, StreamOperation& so, StartMode start_mode)
        : base(buffer_size, start_mode), so(so)
    { }
...
{
public:
    pull(unsigned_type buffer_size, StreamOperation& so, StartMode start_mode = STXXL_START_PIPELINE_DEFERRED_DEFAULT)
...
protected:
    ...
    using basic_pull<StreamOperation>::start_mode;
...
public:
    ...
    //! \param buffer_size Total size of the buffers in bytes.
    //! \param so Input stream operation.
    basic_push(unsigned_type buffer_size, StreamOperation& so, StartMode start_mode)
        : base(buffer_size, start_mode), so(so)
    { }
...
protected:
    ...
    using basic_push<StreamOperation>::start_mode;

    //! Asynchronous method that keeps trying to push from the outgoing buffer.
